...
export class CartItemController {
  static async addToCart(c: Context): Promise<void> {
    try {
  ...
  static async updateCartItem(c: Context): Promise<void> {
    try {
      const body = await c.req.json();
  ...
  static async removeCartItem(c: Context): Promise<void> {
    try {
      ...
      successResponse(c, StatusCodes.OK, 'Item keranjang berhasil dihapus');
    } catch (error) {
      throw error;
    }
  }
