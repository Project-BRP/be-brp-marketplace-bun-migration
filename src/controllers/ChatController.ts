...
export class ChatController {
  ...
  static async getAdminUnreadTotal(c: Context): Promise<Response> {
    try {
      ...
      return successResponse(c, StatusCodes.OK, 'Total unread untuk admin', result)
    } catch (error) {
      throw error
    }
  }
  ...
  static async getUserUnreadTotal(c: Context): Promise<Response> {
    try {
      const result = await ChatService.getUserUnreadTotal(c.get('user')!.userId)
      return successResponse(c, StatusCodes.OK, 'Total unread untuk user', result)
    } catch (error) {
      throw error
    }
  }
  ...
  static async getUserPresence(c: Context): Promise<Response> {
    try {
      ...
      return successResponse(c, StatusCodes.OK, 'Status user', { userId, isOnline })
    } catch (error) {
      throw error
    }
  }
  ...
  static async getAllRooms(c: Context): Promise<Response> {
    try {
      const request: IGetAllChatRoomsRequest = {
        ...
        page: c.req.query().page ? parseInt(c.req.query().page as string, 10) : null,
        limit: c.req.query().limit ? parseInt(c.req.query().limit as string, 10) : null,
      }

      const response = await ChatService.getAllRooms(request)
      return successResponse(c, StatusCodes.OK, 'Daftar chat room berhasil diambil', response)
    } catch (error) {
      throw error
    }
  }
  ...
  static async getRoomDetail(c: Context): Promise<Response> {
    try {
      ...
      return successResponse(c, StatusCodes.OK, 'Detail chat room berhasil diambil', response)
    } catch (error) {
      throw error
    }
  }
    ...
    try {
      ...
      if (files && files.length > 0) {
        for (const f of files) {
          let saved: { path: string; width?: number | null; height?: number | null } = {
            path: '',
          }
      ...
      return successResponse(c, StatusCodes.CREATED, 'Pesan berhasil dikirim', response)
    } catch (error) {
